    ...
    public void doRender(IEntityMultiPart entity, double x, double y, double z, float rotationYaw, float partialTicks)
    {
        if (Minecraft.getMinecraft().getRenderManager().isDebugBoundingBox())
        {
            ...
            for (EntityPart e : entity.getParts())
                RenderGlobal.drawOutlinedBoundingBox(e.getBoundingBox().offset(-Minecraft.getMinecraft().getRenderManager().viewerPosX, -Minecraft.getMinecraft().getRenderManager().viewerPosY, -Minecraft.getMinecraft().getRenderManager().viewerPosZ), 0xffffff);
