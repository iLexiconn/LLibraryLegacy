 ...
 * Instances can be created by calling {@link AdvancedModelLoader#loadModel(ResourceLocation)} with a class-loadable-path
 *
 * @author cpw
 */
...
public class AdvancedModelLoader {
    ...
    public static IModelCustom loadModel(ResourceLocation resource) throws IllegalArgumentException, ModelFormatException {
        ...
        if (i == -1) {
            LLibrary.logger.warn("The resource name " + resource + " is not valid");
            throw new IllegalArgumentException("The resource name is not valid");
        }
        String suffix = name.substring(i + 1);
        IModelCustomLoader loader = instances.get(suffix);
        if (loader == null) {
            LLibrary.logger.warn("The resource name " + resource + " is not supported");
