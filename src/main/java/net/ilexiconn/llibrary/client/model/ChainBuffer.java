...
/**
 * This is a buffer used to delay a chain of parented boxes by using the yaw and
 * pitch of the entity.
 * 
...
public class ChainBuffer
{
    ...
    /**
     * Rotation amount (rotateY) of the tail buffer. Added when the entity
     * rotates.
     */
    ...
    /**
     * Adds a specific rotation, depending on the entity rotation, to an array
     * that can be later used to animate a chain of parented boxes.
     * (rotateAngleY).
     * 
     * @param maxAngle
     *            is the maximum angle that the tail can have. Try values about
     *            40f to 90f degrees;
     * @param bufferTime
     *            is the number of ticks necessary to start reducing the tail
     *            angle. Try values about 5 to 30 ticks;
     * @param angleDecrement
     *            is the amount of angle that will be reduced each tick. Try
     *            values about 3f degrees;
     * @param divider
     *            reduces the amount of angle added to the buffer. Try values
     *            about 5f.
     * @param entity
     *            is the EntityLivingBase that will be used to animate the tail;
     */
    ...
    /**
     * Adds a rotations in the Y axis depending on the entity rotation using a
     * previous set array. (rotateAngleY).
     * 
     * @param boxes
     *            are the chain of parented boxes to be animated;
     */
    public void applyChainSwingBuffer(MowzieModelRenderer[] boxes)
    {
        if (boxes.length == yawArray.length)
            ...
            System.err.println("Wrong array length being used in the buffer! (Y axis)");
    }
    
    /**
     * Adds a rotations in the X axis depending on the entity rotation using a
     * previous set array. (rotateAngleX).
     * 
     * @param boxes
     *            are the chain of parented boxes to be animated;
     */
    public void applyChainWaveBuffer(MowzieModelRenderer[] boxes)
    {
        if (boxes.length == pitchArray.length)
            ...
            System.err.println("Wrong array length being used in the buffer! (X axis)");
    }
}